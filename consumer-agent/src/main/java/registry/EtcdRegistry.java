...
public class EtcdRegistry implements IRegistry{
    ...
    public void register(String serviceName,int port) throws Exception {
        ...
        String strKey = MessageFormat.format("/{0}/{1}/{2}:{3}:{4}", rootPath, serviceName, IpHelper.getHostIp(), String.valueOf(port),
                System.getProperty("capacity"));
    ...
    public List<Endpoint> find(String serviceName) throws Exception {
        ...
        for (com.coreos.jetcd.data.KeyValue kv : response.getKvs()){
            ...
            String capacity = endpointStr.split(":")[2];

            endpoints.add(new Endpoint(host, port, capacity));
        }
        return endpoints;
    }
}
